...
contract AutheoRewardDistribution is Ownable, ReentrancyGuard, Pausable {
    ...
    mapping(address => mapping(BugCriticality => uint256))
        public userRewardCount;
    ...
    event RewardsPerUserCalculated(
        uint256 lowRewardPerUser,
        uint256 mediumRewardPerUser,
        uint256 highRewardPerUser
    );
    ...
    function setTestnetStatus(bool _status) external onlyOwner {
        ...
        // If testnet is being turned off, calculate fixed reward amounts
        if (!_status) {
            _calculateBugBountyRewardsPerUser();
        }

        emit TestnetStatusUpdated(_status);
    }

    // New function to calculate bug bounty rewards when testnet is turned off
    function _calculateBugBountyRewardsPerUser() private {
        uint256 totalBugBountyAllocation = (totalSupply *
            BUG_BOUNTY_ALLOCATION_PERCENTAGE) / MAX_BPS;

        // Calculate low reward per user (handle division by zero)
        if (totalLowBugBountyUserNumber > 0) {
            lowRewardPerUser =
        ...
        // Calculate medium reward per user (handle division by zero)
        if (totalMediumBugBountyUserNumber > 0) {
            mediumRewardPerUser =
                ((totalBugBountyAllocation * MEDIUM_PERCENTAGE) / 10000) /
                totalMediumBugBountyUserNumber;
        } else {
            mediumRewardPerUser = 0;
        }

        // Calculate high reward per user (handle division by zero)
        if (totalHighBugBountyUserNumber > 0) {
            highRewardPerUser =
    ...
    function transferToken(address recipient, uint256 amount) private {
        if (Autheo.balanceOf(address(this)) < amount)
            revert INSUFFICIENT_BALANCE();
        Autheo.safeTransfer(recipient, amount);
    }
    ...
    function _registerBugBountyUsers(
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            userRewardCount[user][criticality]++;
    ...
    function __bugBountyClaim(address _user) private {
        ...
        transferToken(_user, totalRewardAmount);
    ...
    function __contractDeploymentClaim(address _user) private {
        ...
        transferToken(_user, totalReward);
        emit Claimed(
            string.concat(
    ...
    function __claimDappRewards(address _user) private {
        ...
        transferToken(_user, rewardAmount);
        emit Claimed(
            string.concat(
                "Dapp User Reward - ",
                Strings.toString(actingMonths),
